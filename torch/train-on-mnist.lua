...
if opt.network == '' then
   ...
   if opt.model == 'convnet' then
      ...
      -- model:add(nn.SpatialConvolution(1, 6, 5, 5)) -- output 28x28
      -- model:add(nn.Tanh())
      -- model:add(nn.SpatialSubSampling(6, 2, 2, 2, 2)) --output 14x14
      -- model:add(nn.Tanh())
      -- model:add(nn.SpatialConvolution(6, 16, 5, 5)) -- output 10x10
      -- model:add(nn.Tanh())
      -- model:add(nn.SpatialSubSampling(16, 2, 2, 2, 2)) -- output 5x5
      -- model:add(nn.Tanh())
      -- model:add(nn.Reshape(16*5*5))
      -- model:add(nn.Linear(16*5*5, 120))
      -- model:add(nn.Linear(120, noutput))
      -- model:add(nn.Copy('torch.CudaTensor', 'torch.FloatTensor'))
      -- torch.setdefaulttensortype('torch.FloatTensor')
...
function train(dataset)
   ...
   for t = 1,dataset:size(),opt.batchSize do
      ...
      local feval = function(x)
         ...
         local f = criterion:forward(outputs:float(), targets)

         -- estimate df/dW
         local df_do = criterion:backward(outputs:float(), targets)
...
while epoch < 10 do
   ...
   print("Runtime: " .. ((sys.clock() - start_time)) .. 's')
...
print("Total runtime: " .. ((sys.clock() - start_time)) .. 's')